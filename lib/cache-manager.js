...
export async function getArtistRecommendationsWithCache(artistName, artistId, forceRefresh = false, userId = null) {
  try {
    ...
    console.log('Cache miss for artist:', artistName, 'Fetching fresh data...');
    ...
    if (cacheError) {
      console.error('Error caching artist recommendations:', cacheError);
    }
    ...
    console.error('Error in getArtistRecommendationsWithCache:', error);
    throw error;
  }
}
...
export async function cacheFeaturedArtists(artists) {
  ...
  for (const artist of artists) {
    try {
      console.log(`Caching featured artist: ${artist.name}`);
      ...
      console.error(`Error caching featured artist ${artist.name}:`, error);
